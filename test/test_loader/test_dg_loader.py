...
def test_iteration_equal_unit_batch_1(batch_unit, drop_last):
    ...
    for batch in loader:
        ...
        assert batch_num < 5
        assert len(batch) == 1
        assert batch.to_events() == [e for e in events if e.time == batch_num]
        batch_num += 1
    assert batch_num == 5
...
def test_iteration_non_ordered_dg_non_ordered_batch_with_conversion_and_time_delta_values_greater_than_1(
    ...
    events = [
        EdgeEvent(time=0, edge=(1, 2)),  # Batch 1
        EdgeEvent(time=0, edge=(2, 3)),  # Batch 1
        EdgeEvent(time=1, edge=(3, 4)),  # Batch 1
        EdgeEvent(time=1, edge=(4, 5)),  # Batch 1
        EdgeEvent(time=1, edge=(5, 6)),  # Batch 1
        EdgeEvent(time=12, edge=(6, 7)),  # Batch 2
        EdgeEvent(time=18, edge=(7, 8)),  # Batch 2
        EdgeEvent(time=24, edge=(8, 9)),  # Batch 3
        EdgeEvent(time=24, edge=(9, 10)),  # Batch 3
    ]
    dg = DGraph(events, time_delta=TimeDeltaDG(TimeDeltaUnit.SECOND, value=10))
    loader = DGDataLoader(
        dg, batch_size=2, batch_unit=TimeDeltaUnit.MINUTE, drop_last=drop_last
    )
...
def test_iteration_non_ordered_dg_non_ordered_batch_unit_too_granular():
    ...
    dg = DGraph(events, time_delta=TimeDeltaDG(TimeDeltaUnit.SECOND, value=30))
    with pytest.raises(ValueError):
