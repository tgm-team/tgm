...
def test_hook_reset_state(basic_sample_graph):
    ...
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_1)
    assert seed_nids.shape == (1, 2)
    assert seed_nids[0][0] == 0
    assert seed_nids[0][1] == 1
    assert nbr_nids.shape == (1, 2, 1)
    assert nbr_nids[0][0][0] == PADDED_NODE_ID
    assert nbr_nids[0][1][0] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (1, 2, 1)
    assert nbr_edge_time[0][0][0] == _PADDED_TIME_ID
    assert nbr_edge_time[0][1][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (1, 2, 1, 1)  # 1 feature per edge
    assert nbr_edge_x[0][1][0][0] == nbr_edge_x[0][0][0][0] == _PADDED_FEAT_ID

    batch_2 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_2)
    assert seed_nids.shape == (1, 2)
    assert seed_nids[0][0] == 0
    assert seed_nids[0][1] == 2
    assert nbr_nids.shape == (1, 2, 1)
    assert nbr_nids[0][0][0] == 1
    assert nbr_nids[0][1][0] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (1, 2, 1)
    assert nbr_edge_time[0][0][0] == 1
    assert nbr_edge_time[0][1][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (1, 2, 1, 1)  # 1 feature per edge
    assert nbr_edge_x[0][0][0][0] == 1.0
    assert nbr_edge_x[0][1][0][0] == _PADDED_FEAT_ID

    batch_3 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_3)
    assert seed_nids.shape == (1, 2)
    assert seed_nids[0][0] == 2
    assert seed_nids[0][1] == 3
    assert nbr_nids.shape == (1, 2, 1)
    assert nbr_nids[0][0][0] == 0
    assert nbr_nids[0][1][0] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (1, 2, 1)
    assert nbr_edge_time[0][0][0] == 2
    assert nbr_edge_time[0][1][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (1, 2, 1, 1)  # 1 feature per edge
    assert nbr_edge_x[0][0][0][0] == 2.0
    assert nbr_edge_x[0][1][0][0] == _PADDED_FEAT_ID

    batch_4 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_4)
    assert seed_nids.shape == (1, 2)
    assert seed_nids[0][0] == 2
    assert seed_nids[0][1] == 0
    assert nbr_nids.shape == (1, 2, 1)
    assert nbr_nids[0][0][0] == 3
    assert nbr_nids[0][1][0] == 2
    assert nbr_edge_time.shape == (1, 2, 1)
    assert nbr_edge_time[0][0][0] == 3
    assert nbr_edge_time[0][1][0] == 2
    assert nbr_edge_x.shape == (1, 2, 1, 1)  # 1 feature per edge
    assert nbr_edge_x[0][0][0][0] == 5.0
    assert nbr_edge_x[0][1][0][0] == 2.0
...
def _nbrs_2_np(batch: DGBatch) -> List[np.ndarray]:
    ...
    assert hasattr(batch, 'seed_nids')
    ...
    seed_nids = np.array(batch.seed_nids)
    nbr_nids = np.array(batch.nbr_nids)
    nbr_edge_time = np.array(batch.nbr_edge_time)
    nbr_edge_x = np.array(batch.nbr_edge_x)
    return [seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x]
...
def test_init_basic_sampled_graph_directed_1_hop(basic_sample_graph):
    ...
    assert nbr_edge_time[0][1][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (1, 2, 1, 1)  # 1 feature per edge
    assert nbr_edge_x[0][0][0][0] == _PADDED_FEAT_ID
    assert nbr_edge_x[0][1][0][0] == _PADDED_FEAT_ID
...
def test_recency_exceed_buffer(recency_buffer_graph):
    ...
    batch_1 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_1)
    assert seed_nids.shape == (1, 4)
    assert nbr_nids.shape == (1, 4, 2)
    assert nbr_nids[0][0][0] == PADDED_NODE_ID
    assert nbr_nids[0][0][1] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (1, 4, 2)
    assert nbr_edge_time[0][0][0] == _PADDED_TIME_ID
    assert nbr_edge_time[0][0][1] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (1, 4, 2, 1)  # 1 feature per edge
    ...
    assert nbr_edge_time.shape == (1, 4, 2)
    assert nbr_edge_time[0][0][0] == _PADDED_TIME_ID
    assert nbr_edge_time[0][0][1] == 1
    assert nbr_edge_x.shape == (1, 4, 2, 1)  # 1 feature per edge

    for batch in batch_iter:
        seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch)
        assert nbr_nids.shape == (1, 4, 2)
        assert nbr_edge_time.shape == (1, 4, 2)
        assert nbr_nids[0][0][0] == nbr_edge_time[0][0][0] + 1
        assert nbr_nids[0][0][1] == nbr_edge_time[0][0][1] + 1
        assert nbr_edge_x[0][0][0][0] == nbr_edge_time[0][0][0] + 1
        assert nbr_edge_x[0][0][1][0] == nbr_edge_time[0][0][1] + 1
...
def test_2_hop_graph(two_hop_basic_graph):
    ...
    batch_1 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_1)
    assert seed_nids.shape == (2, 2)  # 2 hop, each has 2 node
    assert seed_nids[0][0] == 0
    assert seed_nids[0][1] == 1
    assert nbr_nids.shape == (2, 2, 1)
    assert nbr_nids[0][0][0] == PADDED_NODE_ID
    assert nbr_nids[1][0][0] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (2, 2, 1)
    assert nbr_edge_time[0][0][0] == _PADDED_TIME_ID
    assert nbr_edge_time[1][0][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (2, 2, 1, 1)  # 1 feature per edge

    batch_2 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_2)
    assert seed_nids.shape == (2, 2)
    assert nbr_nids.shape == (2, 2, 1)
    assert nbr_edge_time.shape == (2, 2, 1)
    assert nbr_edge_x.shape == (2, 2, 1, 1)  # 1 feature per edge
    ...
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_5)
    assert seed_nids.shape == (2, 2)
    assert nbr_nids.shape == (2, 2, 1)
    assert nbr_edge_time.shape == (2, 2, 1)
    assert nbr_edge_x.shape == (2, 2, 1, 1)  # 1 feature per edge
    ...
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_6)
    assert seed_nids.shape == (2, 2)
    assert nbr_nids.shape == (2, 2, 1)
    assert nbr_edge_time.shape == (2, 2, 1)
    assert nbr_edge_x.shape == (2, 2, 1, 1)  # 1 feature per edge
...
def test_tgb_non_time_respecting_negative_neighbor_sampling_test(
    ...
    batch_1 = next(batch_iter)
    seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_1)
    assert seed_nids.shape == (2, 6)  # 2 hop, each has 2 node
    assert seed_nids[0][0] == 0
    assert seed_nids[0][1] == 1
    assert seed_nids[0][2] == 2
    assert seed_nids[0][3] == 3
    assert seed_nids[0][4] == 4
    assert seed_nids[0][5] == 5
    assert nbr_nids.shape == (2, 6, 1)
    assert nbr_nids[0][0][0] == PADDED_NODE_ID
    assert nbr_nids[1][0][0] == PADDED_NODE_ID
    assert nbr_edge_time.shape == (2, 6, 1)
    assert nbr_edge_time[0][0][0] == _PADDED_TIME_ID
    assert nbr_edge_time[1][0][0] == _PADDED_TIME_ID
    assert nbr_edge_x.shape == (2, 6, 1, 1)  # 1 feature per edge
...
def test_no_edge_feat_recency_nbr_sampler(no_edge_feat_data):
    ...
    with hm.activate('unit'):
        ...
        seed_nids, nbr_nids, nbr_edge_time, nbr_edge_x = _nbrs_2_np(batch_1)
        assert seed_nids.shape == (1, 6)
        assert nbr_nids.shape == (1, 6, 1)
        assert nbr_edge_time.shape == (1, 6, 1)
        assert nbr_edge_x.shape == (1, 6, 1, 0)  # No edge features
...
def test_node_only_batch_recency_nbr_sampler(node_only_data):
    ...
    with hm.activate('unit'):
        ...
        assert nbr_nids.shape == (1, 6, 1)
        assert nbr_edge_time.shape == (1, 6, 1)
        assert nbr_edge_x.shape == (1, 6, 1, 1)

        batch_2 = next(batch_iter)
        assert isinstance(batch_2, DGBatch)
        torch.testing.assert_close(
            batch_2.seed_nids[0], torch.empty(0, dtype=torch.int32)
        )
        torch.testing.assert_close(
            batch_2.nbr_nids[0], torch.empty(0, dtype=torch.int32)
        )
        torch.testing.assert_close(
            batch_2.nbr_edge_time[0], torch.empty(0, dtype=torch.int64)
        )
        torch.testing.assert_close(
            batch_2.nbr_edge_x[0], torch.empty((0, 1), dtype=torch.float32)
        )
