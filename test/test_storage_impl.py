...
def test_append_single_event(DGStorageImpl):
    events = [
        NodeEvent(t=1, src=2, msg=torch.rand(5)),
        EdgeEvent(t=1, src=2, dst=2, msg=torch.rand(6)),
    ]
    storage = DGStorageImpl(events)

    new_events = [NodeEvent(t=5, src=4, msg=torch.rand(5))]
...
def test_append_multiple_events(DGStorageImpl):
    ...
    new_events = [
        NodeEvent(t=5, src=4, msg=torch.rand(5)),
        EdgeEvent(t=5, src=2, dst=4, msg=torch.rand(6)),
    ]
    storage.append(new_events)
...
def test_append_incompatible_node_feature_dimension(DGStorageImpl):
    ...
    new_events = [
        NodeEvent(t=5, src=4, msg=torch.rand(6)),  # Bad shape
        EdgeEvent(t=5, src=2, dst=4, msg=torch.rand(6)),
    ]
    with pytest.raises(ValueError):
        storage.append(new_events)
...
def test_append_incompatible_edge_feature_dimension(DGStorageImpl):
    events = [
        NodeEvent(t=1, src=2, msg=torch.rand(5)),
        EdgeEvent(t=1, src=2, dst=4, msg=torch.rand(6)),
    ]
    storage = DGStorageImpl(events)

    new_events = [
        NodeEvent(t=5, src=4, msg=torch.rand(5)),
        EdgeEvent(t=5, src=2, dst=4, msg=torch.rand(5)),  # Bad shape
    ]
    with pytest.raises(ValueError):
        storage.append(new_events)
