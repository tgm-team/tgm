...
def train(loader: DGDataLoader, model: nn.Module, opt: torch.optim.Optimizer) -> float:
    ...
    for batch in tqdm(loader):
        ...
        model.memory.detach_memory()  # Detach memory so we don't backpropagate to the start of time
    return total_loss


@torch.no_grad()
def eval(loader: DGDataLoader, model: nn.Module, metric: str, evaluator: any) -> dict:
    model.eval()
    for batch in tqdm(loader):
        pred_label = model(batch)
        np_pred = pred_label.cpu().detach().numpy()
        np_true = batch.node_labels.cpu().detach().numpy()
        input_dict = {
            'y_true': np_true,
            'y_pred': np_pred,
            'eval_metric': [metric],
