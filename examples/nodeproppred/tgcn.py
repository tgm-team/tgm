...
def eval(
    ...
    h_0 = h_0.detach()
    return float(np.mean(perf_list)), h_0
...
for epoch in range(1, args.epochs + 1):
    ...
    val_ndcg, h_0 = eval(val_loader, h_0, encoder, decoder, evaluator)
    log_metric('Loss', loss, epoch=epoch)
    log_metric(f'Validation {METRIC_TGB_NODEPROPPRED}', val_ndcg, epoch=epoch)

    if val_ndcg > best_val:
        best_val = val_ndcg
        test_ndcg, h_0 = eval(test_loader, h_0, encoder, decoder, evaluator)
