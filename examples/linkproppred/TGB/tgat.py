...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        assert self.query_dim % n_heads == 0, (
            'The sum of node_feat_dim and time_feat_dim should be divided by n_heads!'
        )

        self.head_dim = self.query_dim // n_heads
        self.query_projection = nn.Linear(
            self.query_dim, n_heads * self.head_dim, bias=False
        )
        ...
        self.residual_fc = nn.Linear(n_heads * self.head_dim, self.query_dim)
    ...
    def forward(
        ...
        # Tensor, shape (batch_size, 1, n_heads * self.head_dim), where n_heads * self.head_dim is equal to query_dim
