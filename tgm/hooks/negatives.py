    ...
    def __call__(self, dg: DGraph, batch: DGBatch) -> DGBatch:
        ...
        try:
            neg_batch_list = self.neg_sampler.query_batch(
                batch.src, batch.dst, batch.edge_event_time, split_mode=self.split_mode
            )
        ...
        batch.neg_time = torch.randint(  # type: ignore
            int(batch.edge_event_time.min().item()),
            int(batch.edge_event_time.max().item()) + 1,
...
class TGBTHGNegativeEdgeSamplerHook(StatelessHook):
    ...
    requires = {'src', 'dst', 'edge_event_time', 'edge_type'}
    produces = {'neg', 'neg_batch_list', 'neg_time'}
