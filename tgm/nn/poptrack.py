...
class tCoMemPredictor:
    def __init__(
        ...
        # ts_min = float(ts.min().item())
        # ts_max = float(ts.max().item())

        self._window_ratio = window_ratio
        # self._window_end = ts_max
        # self._window_start = ts_max - window_ratio * (ts_max - ts_min)
        self._window_start, self._window_end = ts.min(), ts.max()
        self._window_size = self._window_end - self._window_start

        self.device = src.device

        self.node_to_recent_dests: DefaultDict[int, Deque[Tuple[float, int]]] = (
            defaultdict(lambda: deque(maxlen=k))
        )
    ...
    def update(
        self, src: torch.Tensor, dst: torch.Tensor, ts: torch.Tensor, decay: float
    ) -> None:
        ...
        """
        self._check_input_data(src, dst, ts)
        # self._window_end = torch.max(self._window_end, ts.max().item())
        # self._window_end = max(self._window_end, float(ts.max().item()))
        ...
        for src_, dst_, ts_ in zip(src, dst, ts):
            ...
            self.node_to_co_occurrence[src_][dst_] = (
                self.node_to_co_occurrence[src_].get(dst_, 0) + 1
            )
            self.node_to_co_occurrence[dst_][src_] = (
                self.node_to_co_occurrence[dst_].get(src_, 0) + 1
            )
    ...
    def __call__(
        ...
        # recent = self.node_to_recent_dests.get(int(query_src), [])
        recent: Deque[Tuple[float, int]] = self.node_to_recent_dests.get(
            int(query_src), deque()
        )
