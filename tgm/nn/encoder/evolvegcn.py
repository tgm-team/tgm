...
class EvolveGCNO(torch.nn.Module):
    ...
    Networks for Dynamic Graph." <https://arxiv.org/abs/1902.10191>`.
    ...
    def reinitialize_weight(self) -> None:
        self.weight = None
    ...
    def forward(
        ...
        edge_weight: torch.FloatTensor = None,  # type: ignore
    ) -> torch.FloatTensor:
        ...
        X = self.conv_layer(self.weight.squeeze(dim=0), X, edge_index, edge_weight)  # type: ignore
        return X
...
class EvolveGCNH(torch.nn.Module):
    ...
    def _create_layers(self) -> None:
        self.ratio = self.in_channels / self.num_nodes
