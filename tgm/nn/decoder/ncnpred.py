...
def _has_torch_sparse() -> bool:
    r"""Check if torch_sparse is available."""
    available = False
    try:
...
class NCNPredictor(torch.nn.Module):
    ...
    Args:
        ...
        use_torch_sparse (bool) : whether to use torch_sparse
    ...
    def __init__(
        ...
        has_torch_sparse = _has_torch_sparse()
        ...
        if use_torch_sparse and not has_torch_sparse:
            raise DependencyError(
                'Forcing to use torch-sparse failed due to not yet installing torch_sparse. Please install torch-sparse or use torch version.'
            )
    ...
    def get_cn_emb_torch_sparse(
        ...
        r"""Obtain the CNs embeddings for each node pair in a batch (Torch Sparse version).
    ...
    def forward(
        ...
        if self.use_torch_sparse:
            cn_emb = self.get_cn_emb_torch_sparse(
                x, edge_index, tar_ei, cn_time_decay, time_info
            )
        else:
            cn_emb = self.get_cn_emb_torch(
                x, edge_index, tar_ei, cn_time_decay, time_info
            )
